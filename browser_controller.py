...
async def initialize(browser_page):
    ...
    page.on("popup", lambda popup: _handle_new_tab(popup))
...
async def analyze_page():
    """
    Analyzes the current page and extracts all visible text and interactive elements.
    
    This tool scans the entire DOM structure of the current webpage and identifies all visible 
    text content and interactive elements (buttons, links, inputs, etc.). It returns:
    
    1. Structured output with numbered elements like [1][button]Submit
    2. Complete text content preserving document structure
    3. Builds an internal map of all elements for precise interaction
    
    Use this tool when:
    - First navigating to a new page to understand its content
    - When elements are difficult to find or interact with
    - After page state changes to see updated content
    
    Returns:
        str: Formatted page content with interactive elements marked with IDs
    """
    ...
    try:
        ...
        if len(formatted_result) > 4000:
            print("truncating result to 4000 characters:",formatted_result[:4000])
...
@tool
async def visual_click(target_description):
    """
    Clicks on a webpage element using precise targeting based on element properties.
    
    This tool simulates a human mouse click on an element by:
    1. Identifying the target element using the provided description
    2. Moving the cursor naturally to the element's position
    3. Performing a click action with proper timing
    
    Parameters:
            - String object: {"id": "5", "type": "button", "text": "Submit"} (most precise)
            - Element ID pattern: [3][button]Submit (from analyze_page output)
            - Natural language: "Sign in button" (least precise)
    
    Best practices:
    - Always use element IDs from analyze_page when possible
    - Include element type (button, link, input) for better accuracy
    - For form interactions, always click on a field before typing into it
    
    Returns:
        str: Result of the click operation, success message or error details
    """
    try:
        ...
        if target_id is not None and page_elements:
            try:
                ...
                if 0 <= element_index < len(page_elements):
                    ...
                    result = page_elements[element_index]
        ...
        if not result:
            ...
            result = await _find_element(target_type, target_text, is_structured, target_description=target_description)
        ...
        if not result:
            ...
            result = await _find_element(target_type, target_text, is_structured, relaxed=True)
        ...
        if not result.get('inViewport', False):
            x, y = await _scroll_to_element(result)
        
        # Perform the click
        return await _perform_click(x, y, result)
...
async def _scroll_to_element(element):
    """
    ...
    try:
        ...
        element_coords = await page.evaluate("""
        ...
        if element_coords.get('found', False):
            ...
            if not element_coords.get('inViewport', False):
                ...
                # Get final position
                final_coords = await page.evaluate("""
                    ({tagName, text}) => {
...
async def _perform_click(x, y, element_info):
...
async def keyboard_action(input_text):
    """
    Simulates keyboard input for typing text or pressing special keys.
    
    This tool enables realistic keyboard interactions by:
    1. Typing text with natural timing variations
    2. Executing special key presses (Enter, Tab, arrows, etc.)
    3. Performing key combinations (Ctrl+A, Shift+Tab, etc.)
    
    Parameters:
        input_text (str): The text to type or key command to execute.
            - For regular text: "Hello World"
            - For special keys: "enter", "tab", "escape", "up", "down", "f5"
            - For key combinations: "ctrl+a", "shift+tab", "ctrl+v"
            - For key sequences: "hello, enter, tab" (comma or semicolon separated)
    
    Best practices:
    - Always click on input fields before typing (using visual_click first)
    - For form navigation, use "tab" to move between fields
    - For text editing, use "ctrl+a" to select all before typing new content
    
    Returns:
        str: Description of the keyboard action performed
    """
    try:
        ...
        if "," in input_text or ";" in input_text:
            ...
            for single_key in key_sequence:
                ...
                result = await _execute_single_key_action(single_key, special_keys)
                results.append(result)
                await asyncio.sleep(0.3)  # Brief pause between keys
            
            return "Executed key sequence: " + " → ".join(results)
        else:
            # Single key/text handling
            normalized_input = input_text.lower()
            return await _execute_single_key_action(normalized_input, special_keys)
            
    except Exception as e:
        return f"Error with keyboard action: {str(e)}"

async def _execute_single_key_action(key_input, special_keys):
...
async def navigate(url):
    """
    Navigates the browser to a specified URL with error handling and protocol verification.
    
    This tool simulates a user typing in a URL and visiting a website by:
    1. Normalizing and cleaning the provided URL
    2. Adding https:// protocol if missing
    3. Handling navigation with appropriate timeouts and error recovery
    
    Parameters:
        url (str): The URL to navigate to. Can be in several formats:
            - Full URL with protocol: "https://www.example.com"
            - Domain only: "example.com" (https:// will be added automatically)
            - With path: "example.com/products" (https:// will be added automatically)
    
    Use this tool when:
    - Starting a new task that requires visiting a website
    - Switching between different websites
    - Needing to refresh or reload the current page (navigate to the same URL)
    
    Returns:
        str: Status message indicating successful navigation or error details
    """
    try:
...
async def scroll(direction="down"):
    """
    Scrolls the page in the specified direction using virtual mouse wheel movement.
    
    This tool simulates a human scrolling action by:
    1. Moving the virtual cursor to the center of the viewport
    2. Performing smooth, gradual scrolling with natural pauses
    3. Using appropriate scrolling methods based on the specified direction
    
    Parameters:
        direction (str): The direction to scroll. Options include:
    ...
    Use this tool when:
    - Exploring content below the fold of a page
    - Looking for elements not currently visible in the viewport
    - Navigating long forms or article pages
    - Checking for dynamically loaded content on scroll
    
    Returns:
        str: Status message indicating the scroll direction and result
    """
    try:
...
@tool
async def search_for(query):
    """
    Performs a web search using Google with natural browser interactions.
    
    This tool simulates the complete search process by:
    1. Navigating to Google if not already there
    2. Interacting with the search box using mouse and keyboard
    3. Typing the query with realistic timing
    4. Submitting the search and waiting for results
    
    Parameters:
        query (str): The search term or phrase to look for
    
    Use this tool when:
    - Starting a new information-gathering task
    - Researching a topic without a specific URL in mind
    - Looking for options or alternatives for shopping
    - Finding specific websites when you don't know the exact URL
    
    Returns:
        str: Confirmation message with search details or error information
    """
    try:
        ...
        if ("google" in current_url):
            ...
            search_box = await page.query_selector('input[name="q"], [aria-label="Search"]')
            if (search_box):
                box = await search_box.bounding_box()
                await _natural_mouse_move(box["x"] + box["width"]/2, box["y"] + box["height"]/2)
                await asyncio.sleep(0.3)
                await _virtual_click(box["x"] + box["width"]/2, box["y"] + box["height"]/2)
...
# Helper methods
async def _virtual_click(x, y):
    """Click with the virtual cursor."""
    await virtual_click(page, x, y)

async def _virtual_type(text):
    """Type text character by character with realistic timing."""
    await virtual_type(page, text)
...
def get_browser_tools():
    ...
    """
    # Get all functions in this module that are decorated with @tool
    # We only want the public functions (those that don't start with underscore)
    browser_tools = [
        analyze_page,
        visual_click,
        keyboard_action,
